...
function countSQL3($carecase)    //計算個人個案筆數
{
	$result1 = " SELECT COUNT(*) FROM attention WHERE userid = " .$carecase;
	$data3= mysql_fetch_array(mysql_query($result1));
	return $data3[0];
}
...
//計算所需熱量$weight = 體重, $bmi = BMI, $action = 活動量分級
...
//判斷熱量並秀出燈號$value = 熱量, $max = 最大值, $mid_max = 中間值大, $mid_min = 中間值小, $min = 最小值, $percent = 份數, $kg = 食材重量, $c_kg = 計算重量
