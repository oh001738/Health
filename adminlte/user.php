...
if ($op == 'user' && $_GET['action'] == 'delete' && trim($_GET['id']) != '')
{
    ...
    if ( !in_array(0, $ok) )
    {
        ...
        reDirect(ROOT_URL . "/adminlte/index.php?op=user");
    }else{
...
elseif ( $op == 'user' )
{
    ...
    echo "<form action = '" . ROOT_URL . "/adminlte/index.php?op=user' id = 'searchform' name = 'searchform' method = 'post'>\n";
    echo "<tr>\n";	
	...
	else
	{
	    ...
	    {
    	    echo "<a href = '" . ROOT_URL . "/adminlte/index.php?op=" . $op . $link . "'>第一頁</a>";
        	if ($page > 0)
	        {
    	        $up = $page - 1;
        	    echo "<span style = 'padding-left:10px'><a href = '" . ROOT_URL . "/adminlte/index.php?op=" . $op . "&page=" . $up . $link . "'>上一頁</a></span>";
        	}
        	if ($page < ($total_page - 1))
        	{
            	$next = $page + 1;
	            echo "<span style = 'padding-left:10px'><a href = '" . ROOT_URL . "/adminlte/index.php?op=" . $op . "&page=" . $next . $link . "'>下一頁</a></span>";
    	    }
        	echo "<span style = 'padding-left:10px'><a href = '" . ROOT_URL . "/adminlte/index.php?op=" . $op . "&page=" . ($total_page - 1) . $link. "'>最後一頁</a></span>";
	    }		
...
{
    window.open('./functions/user_history.php?userid=' + userid + '&username=' + username + '&cname=' + c_name,'會員歷史健檢資料','height=450,width=900,toolbar=no,scrollbars=yes,resizable=yes,top=20,left=20');
}
//-->
</script>
