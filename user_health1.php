...
if ($_GET['action'] != 'back')  //判斷是否從下一頁送回
{
	if ($_POST)
	{
		...
		if ( ckusername($username) && trim($username) != '' && $_POST['password']!='')  //檢查使用者帳號是否重複
		{
			$sql = "INSERT INTO user (c_name, e_name, email, username, password, telphone, celphone, address,location, add_time, up_time)VALUES(";
			$sql .= "'" . ckString($_POST['c_name'], 200) . "' , ";
			$sql .= "'" . ckString($_POST['e_name'], 200) . "' , ";
			$sql .= "'" . ckString($_POST['email'], 255) . "' , ";
			$sql .= "'" . ckString($username, 255) . "' , ";
			$sql .= "'" . md5(ckString($_POST['password'], 255)) . "' , ";	//密碼使用MD5編碼
			$sql .= "'" . ckString($_POST['telphone'], 100) . "' , ";
			$sql .= "'" . ckString($_POST['celphone'], 100) . "' , ";
			$sql .= "'" . ckString($_POST['address'], 255) . "' , ";
			$sql .= "'" . ckString($_POST['location'], 2) . "' , ";
			$sql .= "'" . time() . "' , ";
			$sql .= "'" . time() . "')";
			if ( mysql_query($sql) )
			{
				$userid = get_col_value('user', 'id', "WHERE username = '" . $username . "' AND password = '" . md5($_POST['password']) . "'");
				if ( $userid == '' )
				{
					echo "系統發生問題，請洽系統管理員";
					exit;
		...
		elseif(trim($username) == ''||$_POST['password']=='')
		{
			if(trim($username) == '')
			{
				echo "<script language=\"JavaScript\">";
				echo "window.alert('請輸入帳號');";
				echo "location.href('user_add.php');";
				echo "</script>";
				exit;								
			}
			...
			echo "<script language=\"JavaScript\">";
			echo "window.alert('此帳號已經有人使用了，請重新輸入!!');";
			echo "location.href('user_add.php');";
			echo "</script>";
			exit; 
		}
