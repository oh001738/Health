...
if(get_magic_quotes_gpc()){ //系統有無開啟轉換符，如開啟則去掉轉換符
     $name002 = stripslashes(trim($_POST['name001'])); //trim去除首尾空白
     $email002 = stripslashes(trim($_POST['email001']));
     $website002 = stripslashes(trim($_POST['website001']));
     $check002 = stripslashes(trim($_POST['check001']));
	 $text002 = stripslashes(trim($_POST['text001']));
}else{
     $name002 = trim($_POST['name001']); //trim去除首尾空白
     $email002 = trim($_POST['email001']);
     $website002 = trim($_POST['website001']);
...
include_once('./ztype.php');
    ...
    if(!empty($zalarm)){
	 ...
	 $zfun->out_tplreplace($tplrep05, $tplrep06, 'znewpo.htm'); //替換znewpo模板內容
     ...
     $zfun->out_tplreplace($tplrep08, '', 'znewpo.htm'); //替換znewpo模板內容
